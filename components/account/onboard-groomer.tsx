...
/**
 * Founder-only onboarding: prepare a groomer's account and send a single-use
 * invite, plus the invite status list. Rendered inside Settings, gated by
 * useIsFounder(). The privileged actions POST to /api/pipeline/onboard, which
 * independently verifies the founder server-side — the client gate is UI only.
 */
...
export function OnboardGroomer() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!live) {
      ...
      toast("Demo — invite simulated", {
        description: "The live app creates the account and emails the link.",
      });
      setBusinessName("");
      setEmail("");
      setTerms("");
  ...
  return (
    <div className="flex flex-col gap-5">
      <form onSubmit={submit} className="flex flex-col gap-5">
        ...
        <div className="border-t border-DEFAULT pt-4">
          <div className="mb-2 flex items-center justify-between">
            <p className="text-sm font-medium text-ink">Services &amp; prices</p>
            <button
              type="button"
              onClick={addService}
      ...
      {/* Invite status */}
      <div className="border-t border-DEFAULT pt-4">
        <p className="mb-2 text-sm font-medium text-ink">Invites</p>
        {loadingInvites ? (
          <div className="flex items-center justify-center py-8 text-ink-muted">
            <Loader2 className="h-5 w-5 animate-spin" />
          </div>
        ) : invites.length === 0 ? (
          <p className="rounded-xl border border-dashed border-strong bg-surface-sunken px-4 py-6 text-center text-sm text-ink-muted">
          ...
          <ul className="flex flex-col gap-2.5">
            {invites.map((inv) => {
              ...
              return (
                <li
                  key={inv.id}
                  className="flex items-start justify-between gap-3 rounded-xl border border-DEFAULT bg-surface-sunken p-3.5"
                >
